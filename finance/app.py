...
def index():
    ...
    return render_template("index.html", datab = transaction_db, cash = cash)
...
def buy():
    ...
    if request.method == "GET":
        ...
        if not symbol:
            return apology("Give the symbol")

        stock = lookup(symbol.upper())
        if stock == None:
            return apology("Symbol dont exist")

        if shares < 0:
            return apology("Shares aren't allowed")

        transaction_value = shares*stock["price"]

        user_id = session["user_id"]
        user_cash_db = db.execute("SELECT cash FROM users WHERE id = :id", id=user_id)
        usercash = user_cash_db[0]["cash"]

        if usercash < transaction_value:
            return apology("Not enough")

        newcash = usercash - transaction_value

        db.execute("UPDATE users SET cash = ? WHERE id = ?", newcash, user_id)

        date = datetime.datetime.now()
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date)VALUES (?, ?, ?, ?, ?)", user_id, stock["symbol"], shares, stock["price"], date)
        flash("Bought well done")
        return redirect("/")
...
def history():
    ...
    user_id = session["user_id"]
    trans_db = db.execute("SELECT * FROM transactions WHERE user_id = :id", id=user_id)
    return render_template("hist.html", transactions = trans_db)
...
def register():
    ...
    if request.method == "GET":
        ...
        if password != confirmation:
            return apology("Password aren't the same")

        hash = generate_password_hash(password)

        try:
            nu = db.execute("INSERT INTO users (username, hash)VALUES (?, ?)", username, hash)
        except:
